...
function runDocumentation(){
  const installOutput = execSync('npm install @compodoc/compodoc@1.0.5', { encoding: 'utf-8' });
  let output = "";
  try{
    output = execSync('npx compodoc -p tsconfig.doc.json --coverageTest', { encoding: 'utf-8' });
  } catch (err){
    output = err.stdout;
  }
